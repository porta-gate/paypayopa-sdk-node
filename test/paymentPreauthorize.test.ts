...
test('Unit Test - Preauthorize Payment', async () => {
    ...
    expect(mockHttpsCall).toHaveBeenCalledWith(
        expect.objectContaining({ path: '/v2/payments/preauthorize?agreeSimilarTransaction=false' }),
        payload,
        expect.anything(),
    )
...
test('Unit Test - Preauthorize Payment with agreeSimilarTransaction=true', async () => {
    ...
    expect(mockHttpsCall).toHaveBeenCalledTimes(1)
    expect(mockHttpsCall).toHaveBeenCalledWith(
        expect.objectContaining({ path: '/v2/payments/preauthorize?agreeSimilarTransaction=true' }),
        payload,
        expect.anything(),
    )
