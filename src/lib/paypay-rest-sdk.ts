...
class PayPayRestSDK {
    ...
    private createAuthHeader = (method: string, resourceUrl: string, body: unknown) => {
        ...
        if (isempty.includes(jsonified)) {
            ...
            payloadDigest = algo.MD5.create().update(contentType).update(jsonified).finalize().toString(enc.Base64)
        }
    ...
    private paypaySetupOptions = (endpoint: Endpoint, input: any) => {
        ...
        const path = method === 'GET' || method === 'DELETE' ? this.fillPathTemplate(endpoint.path, input) : endpoint.path
